...
def generateEC(attr,key_list,sax_list,tw_sax_list,alpha,alpha_sizes,letter_map_list,TW,tw,age=None,activity_level=None,arm_filepath=None,other_index=None,quick=False,relative_tw=False):
    if ("Activity" not in key_list and tw_sax_list != None and len(tw_sax_list) != 0) or tw == 0.04:
        ...
        if not error:
            comparison_summary, summarizer_list, goal_list = comparison_TW_SAX_summary(summarizer_type,key_list,prev_tw_letters,past_tw_letters,TW,letter_map_list,first_index+1,second_index+1,flag="eval",relative_tw=relative_tw)
...
def generateGC(attr,attr_list,key_list,data_list,sax_list,tw_sax_list,alpha,alpha_sizes,letter_map_list,TW,tw,prev_start_day,start_day,end_day,age=None,activity_level=None,day_list=None,arm_filepath=None,other_index=None,quick=False,relative_tw=False):
    weather_flag = False
    ...
    if not weather_flag and attr != "MyFitnessPalMeals":
        ...
        if not error:
            ...
            comparison_summary, summarizer_list, goal_list = comparison_TW_SAX_summary(summarizer_type,key_list,prev_tw_letters,past_tw_letters,TW,letter_map_list,first_index+1,second_index+1,age=age,activity_level=activity_level,relative_tw=relative_tw)
    ...
    if "Activity" not in key_list and TW != None and TW != "hours":
        ...
        if cluster_data != None and len(cluster_data) != None:
            ...
            if quantifier_list != None:
                ...
                return [cluster_summary, truth, t2, t3, coverage, t4, length, simplicity, tw_index, cluster_data, indices_, clusters, summarizers, summ_indices]
    if quick:
        return [None]*4
    if quick_prov:
        return [None]*5
...
def generateDB(attr,key_list,sax_list,summarizer_7,alpha,alpha_sizes,letter_map_list,alphabet_list,tw,TW,age,activity_level,date_column,arm_filepath=None,quick=False,goals=None,constraint=False):
    ...
    if len(summaries) != 0:
        ...
        if quick:
            return [summaries,truth_list]

        return [summaries, truth_list, t2_list, t3_list, coverage_list, t4_list, length_list, simplicity_list, summarizers]

    return [None]*9

def generateGA(attr,df_list,key_list,sax_list,summarizer_7,start_day,end_day,alpha,alpha_sizes,letter_map_list,alphabet_list,tw,TW,age,activity_level,date_column,arm_filepath=None,quick=False):
    diet = "2000-cal"
    ...
    if tw > 0 and attr != "MyFitnessPalMeals":
        ...
        goal_dict = goal_assistance(diet,last_weeks)
    ...
    summary = "You have been doing well at maintaining the " + diet + " diet."
    if quick:
        return [summary]
...
def get_single_SAX_summary(attr_list,letter_list,letter_map_list,alpha_sizes,TW,tw_size=None,past_tw=None,age=None,activity_level=None):
    ...
    if type(letter_list) is str and len(letter_list) > 1:
        print(letter_list)
        input("Check get_single_SAX_summary input")
...
def comparison_TW_SAX_summary(summarizer_type,attr_list,prev_letters,curr_letters,TW,letter_map,first_index,second_index,tw=None,flag=None,age=None,activity_level=None,relative_tw=False):
    '''
    Inputs:
    - summarizer_type: the domain of the summarizer we are looking at (e.g., step counts)
    - attr: the attribute
    ...
    if flag == "eval":
        ...
        for i in range(len(attr_list)):
            ...
            if i == len(attr_list)-1:
                ...
                if summarizer_list[i] == "about the same" or summarizer_list[i] == "aboutthesame":
                    summary += " as "
                else:
                    summary += " than "
    ...
    word = " than"
    if "about the same" in summary or "not do as well" in summary:
        word = " as"
    summary += word + " you did in " + TW[:-1] + " " + str(first_index) + "."
    #else:
...
def get_data_list(index_list,dataset,demo=False,constraints=False,bank=False):
    ...
    if dataset == "Stock Market Data":
        ...
        if demo:
            ...
            data_folder = "data/MyFitnessPal/mfp_data"
            #data_folder = "data/MyFitnessPal/FoodLogs"
...
def get_patterns(sax_list,num_sax,letter_map_list,tw,alpha_sizes,prefix,path,cygwin_path,min_sup,weekdays=None):
    ...
    if not error:
        ...
        spade = ["wsl","spade.exe", "-e", "1", "-i", file_path, "-s", minimum_support, "-o", "-w", TW, "-u", "1","-Z",seq_length]
...
def retrieve_weekdays(attr,date_column,weekday_dict):
    for i in range(len(date_column)):
        ...
        if attr == "Weather":
            ...
            date = date.split('/')
            date = datetime.datetime(int(date[2]),int(date[0]),int(date[1]))
        elif attr == "Step Count" or attr == "Heart Rate" or attr == "ActivFit" or attr == "StepUp":
            date = date.split(" ")
            date = date[0]
            date = date.split('-')
            ...
            date = date.split('/')
            date = datetime.datetime(int(date[2]),int(date[0]),int(date[1]))
        elif attr == "MyFitnessPal" or attr == "MyFitnessPalMeals":
            if type(date) == list:
                date = date[0]
            if '/' in date:
                date = date.split('/')
            ...
            date = datetime.datetime(int(day_info[0]),int(day_info[1]),int(day_info[2]),hour=int(time_info[0]),minute=int(time_info[1]),second=int(time_info[2]))
